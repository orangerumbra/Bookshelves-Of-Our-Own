...
async function loadFromCloud() {
  ...
  try {
    ...
    lib.shelves = shelves.map(s => ({
      id: s.id, name: s.name, expanded: currentlyExpanded.includes(s.id),
...
function render(){
  ...
  lib.shelves.forEach(shelf=>{
    ...
    h+='<div class="bookshelf"><div class="shelf-header" onclick="togShelf(\''+shelf.id+'\')">';
    h+='<div class="shelf-header-left"><span class="shelf-name">'+esc(shelf.name)+'</span><span class="shelf-count">['+cnt+']</span></div>';
    ...
    shelf.layers.forEach(layer=>{
      h+='<div class="shelf-layer"><div class="layer-header">';
      h+='<span class="layer-name" ondblclick="editLayer(\''+shelf.id+'\',\''+layer.id+'\')">'+esc(layer.name)+'</span>';
...
// ─── DRAG & DROP ───
function dStart(e,bid,sid,lid){dragD={bid,sid,lid};e.target.classList.add('dragging');e.dataTransfer.effectAllowed='move'}
...
document.addEventListener('dragend',()=>{document.querySelectorAll('.dragging').forEach(el=>el.classList.remove('dragging'));document.querySelectorAll('.drag-over').forEach(el=>el.classList.remove('drag-over'))});
...
function buildStats(yr){
  ...
  const readWithRating = all.filter(b => b.status === 'read');
...
document.onkeydown=function(e){
  if(e.key==='Escape'){closeModal();closeDet();$('advPanel').classList.remove('open')}
